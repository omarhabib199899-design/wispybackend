...
exports.getBots = asyncHandler(async (req, res) => {
  const bots = await Bot.find({ user: req.user._id, isDeleted: false }).sort({ createdAt: -1 });
...
exports.createBot = asyncHandler(async (req, res, next) => {
  ...
  if (!canCreate) {
    return next(new ErrorResponse(`Your ${req.user.plan} plan allows limited bots. Upgrade to create more.`, 403));
  }

  const {
    name, description, welcomeMessage, fallbackMessage,
    businessName, businessType, businessHours, businessPhone, businessEmail, businessAddress,
    systemPrompt, theme, leadCapture, allowedDomains, language,
  } = req.body;

  if (!name?.trim()) return next(new ErrorResponse('Bot name is required', 400));
...
exports.updateBot = asyncHandler(async (req, res, next) => {
  ...
  const allowed = [
    'name', 'description', 'welcomeMessage', 'fallbackMessage',
    'businessName', 'businessType', 'businessHours', 'businessPhone',
    'businessEmail', 'businessAddress', 'systemPrompt', 'theme',
    'leadCapture', 'allowedDomains', 'isActive', 'language',
...
exports.getEmbedCode = asyncHandler(async (req, res, next) => {
  const bot = await getBot(req.params.id, req.user._id);
  const widgetUrl = process.env.WIDGET_CDN_URL || process.env.RAILWAY_PUBLIC_DOMAIN
...
exports.addQA = asyncHandler(async (req, res, next) => {
  ...
  if (!question?.trim() || !answer?.trim()) return next(new ErrorResponse('Question and answer are required', 400));
  if (bot.knowledge.length >= 200) return next(new ErrorResponse('Maximum 200 knowledge entries reached', 400));

  bot.knowledge.push({ type: 'qa', question: question.trim(), answer: answer.trim() });
...
exports.addText = asyncHandler(async (req, res, next) => {
  ...
  if (!content?.trim()) return next(new ErrorResponse('Content is required', 400));
...
exports.scrapeUrl = asyncHandler(async (req, res, next) => {
  if (!req.user.hasFeature('urlScraping')) {
    return next(new ErrorResponse('URL scraping requires Business plan or higher', 403));
  }
  ...
  if (!rawText) return next(new ErrorResponse('No content found at this URL', 400));
...
exports.getBotStats = asyncHandler(async (req, res, next) => {
  ...
  const todayConvs  = await Conversation.countDocuments({ bot: bot._id, createdAt: { $gte: today } });
  const todayLeads  = await Lead.countDocuments({ bot: bot._id, createdAt: { $gte: today } });
