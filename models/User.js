...
const UserSchema = new mongoose.Schema({
  ...
  // ── Plan & billing ─────────────────────────────────────────────────────────
  plan:          { type: String, enum: ['trial', 'starter', 'business', 'premium'], default: 'trial' },
  trialEndsAt:   { type: Date, default: () => new Date(Date.now() + 14 * 24 * 60 * 60 * 1000) },
  planStatus:    { type: String, enum: ['active', 'cancelled', 'past_due', 'trialing'], default: 'trialing' },
...
UserSchema.methods.canConverse = async function () {
  ...
  if (this.plan !== 'trial') {
    if (this.planStatus === 'cancelled' && this.planExpiresAt && this.planExpiresAt < new Date()) {
